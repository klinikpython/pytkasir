# file: formLapMasPer.py
# desain form laporan Barang Masuk per periode
# tgl_buat: 28 okt 2010 08.12 AM
# tgl_revisi : -
# lisensi: GPL
...
class FormLapMasPer(Toplevel):
    def __init__(self, parent):
        ...
        self.title("Form Laporan Barang Masuk Per Periode")
    ...
    def kopLaporan(self, x1, x2, x3, x4, x5, x6, x7, x8, x9, a, b):
    ...
    def onShow(self, x1, x2, x3, x4, x5, x6, x7, x8, x9, event=None):
        ...
        self.sqlBarmas = """
            ...
            WHERE barang_masuk.no_masuk=detbarang_masuk.no_masuk AND 
                ...
                barang_masuk.kd_pengguna=pengguna.kd_pengguna AND
                ((barang_masuk.tgl_masuk >= '%s') AND 
                (barang_masuk.tgl_masuk <= '%s'));
            """ %(tglAwal, tglAkhir)
        ...
        for data in range(jum):
            ...
            sumTotal += int(bar[data][7])
        
        self.kanvas.create_text(x1+600, 20, 
            text="TOTAL PEMBELIAN :: %s"%(str(sumTotal)), 
...
if __name__ == '__main__':
    ...
    def run():
        ...
        obj = formLapMasPer.FormLapMasPer(root)
